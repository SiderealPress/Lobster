...
def normalize_event(event: dict) -> dict:
    ...
    """
    msg_id = f"{int(time.time() * 1000)}_wa"
    ...
    return {
        ...
        "mentions_lobster": bool(event.get("mentions_lobster")),
        "timestamp": ts,
    }
...
class OutboxHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if event.src_path.endswith(".json"):
            Thread(target=self._process_sync, args=(event.src_path,), daemon=True).start()

    def _process_sync(self, filepath: str) -> None:
        try:
            time.sleep(0.1)
    ...
    for f in sorted(OUTBOX_DIR.glob("*.json")):
        try:
            ...
            if data.get("source", "").lower() == "whatsapp":
                OutboxHandler()._process_sync(str(f))
        except Exception as exc:
