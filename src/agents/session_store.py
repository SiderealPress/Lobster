...
def session_start(
    ...
    Args:
        ...
        task_origin:        Origin of the task: 'user' | 'scheduled' | 'internal'.
                            'user'      — message originated from a real user (Telegram, Slack, etc.).
                            'scheduled' — triggered by a scheduled job or cron task.
                            'internal'  — system-initiated, no user involved
                                          (reconciler, health check, dispatcher, etc.).
                            Defaults to 'user'. Replaces the chat_id=0 sentinel
                            as the canonical way to signal no-user tasks.
