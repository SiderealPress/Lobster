...
2. **If new messages found:**
   - Post GitHub comments and send Telegram notification to Sahar (chat_id=8305714125) # noname
...
3. **If no new messages:**
   ...
   - Do NOT notify Sahar (no-op cycle) # noname
...
Same as `bot-talk-poller`: only notify Sahar when there is genuinely new content. # noname
...
" 2>/dev/null && substep "agent_sessions.idempotency column present (fresh or migrated)" && migrated=$((migrated + 1)) || true
    ...
    # Migration 49: Add inbox-staleness-warn.sh cron entry
