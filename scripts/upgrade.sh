...
M88_PYEOF
    ...
    # Migration 91: Fix on-compact.py hook matcher in settings.json (issue #1947).
...
" 2>/dev/null; then
        ...
        if [ "$_m91_needs_fix" -eq 1 ]; then
            substep "Fixing on-compact.py hook matcher (Migration 91)..."
            local _m91_tmp
            _m91_tmp=$(mktemp)
            python3 - "$CLAUDE_SETTINGS" "$_m91_tmp" << 'M91_PYEOF'
import json, sys
src, dst = sys.argv[1], sys.argv[2]
with open(src) as f:
    data = json.load(f)
...
with open(dst, 'w') as f:
    ...
    f.write('\n')
M91_PYEOF
            if [ $? -eq 0 ] && [ -s "$_m91_tmp" ]; then
                mv "$_m91_tmp" "$CLAUDE_SETTINGS"
                success "Migration 91: fixed on-compact.py hook matcher (matcher='' + removed redundant compact inject-bootup-context entry)"
                migrated=$((migrated + 1))
            else
                rm -f "$_m91_tmp" 2>/dev/null || true
                warn "Migration 91: failed to update $CLAUDE_SETTINGS"
            fi
        else
            substep "Migration 91: on-compact.py hook already uses matcher='' — skipping"
        fi
    else
        substep "Migration 91: settings.json or python3 not found — skipping"
    fi
