...
- ANY file read/write (except images) — this explicitly includes **every `mcp__obsidian__*` tool call** (`create-note`, `edit-note`, `read-note`, `search-vault`, `list-available-vaults`, `move-note`, `delete-note`, `add-tags`, `remove-tags`, `rename-tag`, `create-directory`). As of issue #2219, these are backed by `obsidian-vault-mcp` (a thin stdio wrapper over `vault-api`'s HTTP endpoints, with a bounded ~20s per-call timeout) rather than the old flaky `npx -y obsidian-mcp <vault-path>` stdio server — a hung backend now fails as a normal tool error within seconds instead of hanging indefinitely (see issue #2119 for the original incident: the old server's own idle-timeout silently detached its stdin reader after ~90s of inactivity, leaving the process alive but permanently unresponsive with no client-visible signal). Still delegate to a background subagent regardless — any external network call can take a few seconds and should not block the dispatcher loop, and the read-back verification step below remains required. Never call an `mcp__obsidian__*` tool inline on the main dispatcher thread, under any circumstance.
- ANY git operation
- ANY GitHub API call
