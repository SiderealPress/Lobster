...
so peer Lobster instances can observe this instance's activity.
...
Anti-duplication
----------------
The bot-talk poller reads only messages with a remote sender name (e.g. "AlbertLobster").
This module writes with this instance's MY_LOBSTER_NAME, so there is no echo loop.
...
# BOT_TALK_SENDER — canonical name for this Lobster instance. Override via env var
# or store in ~/lobster-workspace/data/lobster-name.txt.
_lobster_name_file = Path.home() / "lobster-workspace" / "data" / "lobster-name.txt"
BOT_TALK_SENDER: str = (
    os.environ.get("MY_LOBSTER_NAME")
    or (_lobster_name_file.read_text().strip() if _lobster_name_file.exists() else "")
    or _read_config_env("MY_LOBSTER_NAME")
    or "MyLobster"
)
