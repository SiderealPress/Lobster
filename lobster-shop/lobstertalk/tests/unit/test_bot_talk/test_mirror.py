"""
Unit tests for bot_talk mirror module.
...
# Ensure tooling/src/bot_talk is on sys.path so mirror can be imported directly.
_BOT_TALK_DIR = Path(__file__).parent.parent.parent.parent / "tooling" / "src" / "bot_talk"
if str(_BOT_TALK_DIR) not in sys.path:
    sys.path.insert(0, str(_BOT_TALK_DIR))

import mirror as btm
...
class TestBuildHttpPayload:
    ...
    def test_sender_is_configured_lobster_name(self):
        """Sender must be the configured Lobster name (BOT_TALK_SENDER), not hardcoded."""
        payload = btm._build_http_payload("x", "status-update")
        assert payload["sender"] == btm.BOT_TALK_SENDER
        assert payload["sender"]  # non-empty
...
class TestTryHttp:
    def test_returns_true_on_201(self):
        ...
        with patch.object(btm, "BOT_TALK_HTTP_URL", _FAKE_HTTP_URL), \
             patch("mirror.httpx.Client") as mock_client_cls:
            ...
            result = btm._try_http({"sender": "OperatorLobster", "content": "x", "tier": "TIER-BOT", "genre": "status-update"})
...
class TestTrySsh:
    def test_returns_true_on_success(self):
        with patch("mirror.subprocess.run") as mock_run:
    ...
    def test_returns_false_on_exception(self):
        with patch("mirror.subprocess.run", side_effect=Exception("timeout")):
            result = btm._try_ssh("some log line")
        assert result is False
