...
if [ "$(id -u)" = "0" ]; then
    ...
    exec sudo -u lobster \
        ...
        ${LOBSTER_ADMIN_CHAT_ID:+LOBSTER_ADMIN_CHAT_ID="$LOBSTER_ADMIN_CHAT_ID"} \
        bash "$TMP_SCRIPT" "$@"
fi
...
if [ "$NEED_CONFIG" = true ] && [ "$NON_INTERACTIVE" = true ]; then
    # If Telegram credentials are available as env vars, use them directly.
    if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ "${TELEGRAM_BOT_TOKEN}" != "your_bot_token_here" ] \
        && [ -n "${TELEGRAM_ALLOWED_USERS:-}" ]; then
        step "Writing Telegram credentials from environment variables..."
        mkdir -p "$(dirname "$CONFIG_FILE")"
        BOT_TOKEN="$TELEGRAM_BOT_TOKEN"
        USER_ID="$TELEGRAM_ALLOWED_USERS"
        ADMIN_CHAT_ID="${LOBSTER_ADMIN_CHAT_ID:-$USER_ID}"
        cat > "$CONFIG_FILE" << EOF
# Lobster Configuration
# Generated by installer on $(date) (non-interactive - credentials from environment)
...
LOBSTER_ADMIN_CHAT_ID=$ADMIN_CHAT_ID
...
EOF
        success "Telegram credentials written to $CONFIG_FILE"
    else
        warn "Skipping Telegram configuration (non-interactive mode, no credentials in environment)."
        info "Set TELEGRAM_BOT_TOKEN and TELEGRAM_ALLOWED_USERS env vars to configure non-interactively."
...
# Telegram Bot (UNCONFIGURED - set TELEGRAM_BOT_TOKEN and TELEGRAM_ALLOWED_USERS and re-run)
