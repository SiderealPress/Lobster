...
# Create / sync the venv from pyproject.toml
if [ -f "pyproject.toml" ]; then
    uv sync
    success "Python dependencies installed via uv sync"
else
    # Fallback: create venv and install core packages manually
    warn "pyproject.toml not found - creating venv and installing core packages"
    uv venv .venv
    uv pip install mcp python-telegram-bot watchdog python-dotenv slack-bolt psutil
fi
...
if uv pip install fastembed; then
...
if uv pip install sqlite-vec 2>/dev/null; then
...
if [ "$SQLITE_VEC_OK" = false ]; then
    ...
    if uv pip install "sqlite-vec==0.1.7a2" 2>/dev/null; then
        if uv run python -c "import sqlite_vec" 2>/dev/null; then
...
if [ "$SQLITE_VEC_OK" = false ]; then
    ...
    if git clone --quiet --depth 1 https://github.com/asg017/sqlite-vec.git "$_SQLITE_VEC_SRC_DIR" 2>/dev/null; then
        ...
        if make loadable python 2>/dev/null && uv pip install -e . 2>/dev/null; then
            if uv run python -c "import sqlite_vec" 2>/dev/null; then
