...
#   5. on-compact.py uses matcher="" (not matcher="compact") — issue #1947
#   6. No redundant compact-matcher inject-bootup-context entry exists
#
...
check_hook "inject-bootup-context all-sessions (SessionStart, matcher='')" \
...
# Test 6: on-compact.py hook correctness (issue #1947)
...
# Test 7: No redundant compact-matcher inject-bootup-context entry
...
# Test 10: Idempotency — run again and count hooks (no duplicates)
setup_claude_hooks
