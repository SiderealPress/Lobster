...
Fix:
1. task sets `chat_id=1000000000` (ADMIN_CHAT_ID) on inbox messages — the integer
...
End-to-end path traced here:
  lobstertalk-unified runs
    ...
    → writes inbox file with chat_id=1000000000, source="bot-talk", from="AlbertLobster"
  dispatcher picks up inbox file
    → sees source="bot-talk"
    → formats: "📨 From AlbertLobster via LobsterTalk:\n\n<text>"
    → send_reply(chat_id=1000000000, source="telegram", text=...)
...
ADMIN_CHAT_ID = 1000000000  # placeholder — real value is per-install in user config
...
def build_inbox_message(sender: str, content: str, local_identity: str) -> dict[str, Any]:
    ...
    chat_id is ALWAYS the owner's ADMIN_CHAT_ID (1000000000), not the sender name.
...
class TestInboxMessageChatId:
    ...
    def test_chat_id_is_admin_chat_id(self):
        """The core bug: chat_id was set to sender name string. Must be 1000000000."""
