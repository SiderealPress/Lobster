    ...
    def test_categorizes_features(self, mock_run, tmp_path):
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "rev-parse" in cmd:
                ...
                result.stdout = "abc123 feat: add new tool (Test Author,2 hours ago)\ndef456 fix: repair inbox (Test Author,1 hour ago)\n"
            else:
                result.stdout = ""
            return result
    ...
    def test_other_category(self, mock_run, tmp_path):
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "rev-parse" in cmd:
                ...
                result.stdout = "abc123 chore: update deps (Test Author,1 hour ago)\n"
            else:
                result.stdout = ""
            return result
    ...
    def test_safe_auto_plan(self, mock_run, tmp_path):
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "fetch" in cmd:
                ...
                result.stdout = "ccc333 feat: new feature (Test Author,1h ago)\nddd444 docs: update readme (Test Author,2h ago)\n"
            elif "diff" in cmd and "--name-only" in cmd:
                result.stdout = "src/mcp/update_manager.py\nREADME.md\n"
            elif "status" in cmd:
                result.stdout = ""
    ...
    def test_manual_plan_with_breaking_changes(self, mock_run, tmp_path):
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "fetch" in cmd:
                ...
                result.stdout = "ccc333 feat: add migration (Test Author,1h ago)\n"
            elif "diff" in cmd and "--name-only" in cmd:
