...
# On compaction, on-compact.py writes this file proactively so the new session
# is identified before session_start() is called.
...
def _is_fresh_dispatcher_start(hook_input: dict) -> bool:
    ...
    This is a fallback check for the case where session_role.is_dispatcher()
    returns False despite the session being the real dispatcher.  That failure
    mode occurs when write-dispatcher-session-id.py misidentifies the new
    dispatcher as a subagent (issue #1768): it does so when the previous
    dispatcher's JSONL file has a recent mtime, causing _stored_session_is_alive()
    to return True and suppressing the session-ID file update.  The tertiary
    file then contains the old session UUID, which does not match the new session
    UUID, so is_dispatcher() returns False.
...
def main() -> None:
    ...
    #
    # is_dispatcher() uses two state files to detect the dispatcher:
    # 1. Primary: dispatcher-claude-session-id (written by MCP server on session_start)
    # 2. Tertiary: messages/config/dispatcher-session-id (written by write-dispatcher-session-id.py)
    #
    # On a fresh restart, write-dispatcher-session-id.py may misclassify the new
    # dispatcher as a subagent when the previous session's JSONL file has a recent
    # mtime — causing it to skip the session-ID file update.  The tertiary file
    # then contains the old UUID, producing a mismatch → is_dispatcher() returns False.
    #
    # Fallback (issue #1768): if is_dispatcher() returns False but the primary
    # state file is absent AND we are in a LOBSTER_MAIN_SESSION=1 environment,
    # treat this as a fresh dispatcher start.  The primary file is cleared by the
    # MCP server on startup and is absent until the dispatcher calls session_start();
    # at that point, subagents have not yet been spawned, so absent primary file
    # + non-compaction event + LOBSTER_MAIN_SESSION=1 reliably identifies the
    # dispatcher.
